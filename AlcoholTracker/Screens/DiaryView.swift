...
struct DiaryView: View {
        ...
        HStack(alignment: .bottom) {
            VStack(alignment: .leading, spacing: 2) {
                Text(model.selectedDayTitle)
                    .font(Fonts.text(scrolled ? 22 : 32, .bold))
                    .kerning(-0.5)
    ...
    private var remainLine: (text: String, color: Color) {
        ...
        if u == 0 {
            return (L.s("diary_remaining_zero_neutral"), theme.sub)
        }
        if rem > 0 {
            return (
        ...
        if rem == 0 {
            return (L.s("diary_remaining_at_target_neutral"), theme.sub)
        }
        return (
    ...
    private func miniProgress(label: String, value: Double, target: Int) -> some View {
        VStack(spacing: 5) {
            ...
            ThinBar(fraction: value / Double(max(1, target)), color: theme.acc)
        }
        .accessibilityElement(children: .combine)
        .accessibilityLabel(L.f(
            "a11y_mini_progress",
    ...
    private var bacFreeCard: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack {
                Text(L.s("bac_monitor")).font(Fonts.text(15, .semibold))
                Spacer()
                ProBadge { model.openPaywall() }
            }
            HStack(alignment: .firstTextBaseline, spacing: 10) {
                Text(verbatim: "--")
                    .font(Fonts.figure(33))
                    .foregroundStyle(theme.sub)
                HStack(spacing: 6) {
                    PulsingDot(color: theme.sub)
    ...
    private func bacLiveCard(_ bac: BacEstimate) -> some View {
        ...
        case .rising: (theme.elev, theme.amber)
        case .settling: (theme.elev, theme.acc)
        case .clear: (theme.elev, theme.moss)
        }
    ...
    private var dryCard: some View {
        ...
        .padding(22)
        .background(RoundedRectangle(cornerRadius: 22, style: .continuous).fill(theme.elev))
    }
