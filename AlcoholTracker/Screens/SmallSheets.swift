...
struct CalendarSheet: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            SoftButton(title: L.s("action_done"), background: theme.elev, foreground: theme.ink, height: 48) {
                model.closeSheet()
            }
            .padding(.horizontal, 20)
            .padding(.top, 14)
        ...
        if let day = cell.day, let key = cell.key {
            ...
            Button {
                ...
                VStack(spacing: 3) {
                    ...
                    Circle()
                        .fill(logged ? theme.acc : (dry ? theme.moss : .clear))
                ...
                .background(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .fill(isSelected ? theme.elev : .clear)
                        .overlay(
                            RoundedRectangle(cornerRadius: 12, style: .continuous)
                                .strokeBorder(isToday ? theme.acc : .clear, lineWidth: 1.5)
                        )
                )
            ...
            if let entry {
                HStack(spacing: 16) {
                    ...
                    VStack(alignment: .leading, spacing: 3) {
                        Text(entry.name).font(Fonts.figure(21))
                ...
                SoftButton(title: L.s("sheet_entry_relog"), background: theme.elev, foreground: theme.acc) {
                    model.relog(entry)
                }
                .padding(.top, 18)
...
struct UnitsInfoSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            FlowChips {
                formulaChip(L.s("sheet_units_chip_pour"), bg: theme.elev, fg: theme.acc, delay: 0.05)
                operatorText("×")
                formulaChip(L.s("sheet_units_chip_abv"), bg: theme.elev, fg: theme.amber, delay: 0.15)
                operatorText("×")
                formulaChip(L.s("sheet_units_chip_density"), bg: theme.elev, fg: theme.sub, delay: 0.25)
                operatorText("=")
                formulaChip(L.unit("sheet_units_chip_result", UnitsConfig.current.noun(.plural)),
                            bg: theme.elev, fg: theme.moss, delay: 0.4, bold: true)
            }
            .padding(.top, 18)
            ...
            SoftButton(title: L.s("diary_adjust_guideline"), background: theme.elev, foreground: theme.ink, height: 48) {
                model.closeSheet()
                Task {
    ...
    private func formulaChip(_ text: String, bg: Color, fg: Color, delay: Double, bold: Bool = false) -> some View {
        Text(text)
            .font(Fonts.text(13.5, bold ? .bold : .semibold))
            .monospacedDigit()
            .foregroundStyle(fg)
                    ...
                    ForEach(Presets.customDrinkBases, id: \.self) { b in
                        Button {
                            ...
                            Text(b)
                                ...
                                .foregroundStyle(base == b ? .white : theme.sub)
                                .padding(.horizontal, 15)
                                .frame(height: 34)
                                .background(Capsule().fill(base == b ? theme.acc : theme.elev))
                        }
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 1) {
                    Text(L.s("sheet_health_data_title")).font(Fonts.text(14.5, .semibold))
